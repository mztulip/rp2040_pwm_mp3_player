...
/*!
    ...
    @param  twi
            Pointer to an existing AsyncTwoWire instance (e.g. &Wire, the
    ...
    @return ModAdafruit_SSD1306 object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(uint8_t w, uint8_t h, AsyncTwoWire *twi,
                                   int8_t rst_pin, uint32_t clkDuring,
                                   uint32_t clkAfter)
    : Adafruit_GFX(w, h), spi(NULL), wire(twi ? twi : &AsyncWire1), buffer(NULL),
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(uint8_t w, uint8_t h, int8_t mosi_pin,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(uint8_t w, uint8_t h, SPIClass *spi_ptr,
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using software
            ...
            one of the SSD1306_* size defines in ModAdafruit_SSD1306.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
                                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
...
/*!
    @brief  DEPRECATED constructor for I2C SSD1306 displays. Provided for
            ...
            defines in ModAdafruit_SSD1306.h. New code should NOT use this.
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
ModAdafruit_SSD1306::ModAdafruit_SSD1306(int8_t rst_pin)
    : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(&AsyncWire1),
      buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1),
      rstPin(rst_pin) {}

/*!
    @brief  Destructor for ModAdafruit_SSD1306 object.
*/
ModAdafruit_SSD1306::~ModAdafruit_SSD1306(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
/*!
   ...
   method
*/
inline void ModAdafruit_SSD1306::SPIwrite(uint8_t d) {
  if (spi) {
    (void)spi->transfer(d);
  } else {
...
/*!
   ...
   protected function, not exposed (see ssd1306_command() instead).
    ...
    @note
*/
void ModAdafruit_SSD1306::ssd1306_command1(uint8_t c) {
  if (wire) { // I2C
...
/*!
   ...
   transactions. This is a protected function, not exposed.
    ...
    @note
*/
void ModAdafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if (wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint16_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void ModAdafruit_SSD1306::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
/*!
    ...
    @param  periphBegin
            ...
            (e.g. a AsyncTwoWire interface on non-default pins, as can be done
...
*/
bool ModAdafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
                             bool periphBegin) {
  ...
  if (wire) { // Using I2C
    ...
    // AsyncTwoWire begin() function might be already performed by the calling
...
*/
void ModAdafruit_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void ModAdafruit_SSD1306::clearDisplay(void) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void ModAdafruit_SSD1306::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
/*!
   ...
   SSD1306_INVERSE.
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void ModAdafruit_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
                                             uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void ModAdafruit_SSD1306::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
/*!
   ...
   SSD1306_INVERSE.
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void ModAdafruit_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y,
...
*/
bool ModAdafruit_SSD1306::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch (getRotation()) {
    case 1:
...
*/
uint8_t *ModAdafruit_SSD1306::getBuffer(void) { return buffer; }
...
*/
void ModAdafruit_SSD1306::display(void) {
  TRANSACTION_START
...
void ModAdafruit_SSD1306::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void ModAdafruit_SSD1306::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void ModAdafruit_SSD1306::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void ModAdafruit_SSD1306::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void ModAdafruit_SSD1306::stopscroll(void) {
  TRANSACTION_START
...
*/
void ModAdafruit_SSD1306::invertDisplay(bool i) {
  TRANSACTION_START
...
*/
void ModAdafruit_SSD1306::dim(bool dim) {
